...
def rand1d(x):
    ...
    s  = np.random.uniform(0,1,npoints).astype(complex)
...
def main():
    ...
    #psi = ring()
    #psi = grey_soliton_rand2(0.,-5)*grey_soliton_rand2(-0.,5)
    r = rand3()
    plt.imshow(np.real(r), cmap=plt.get_cmap("BuPu"), origin='lower', 
                extent=[-L/2, L/2-dx, -L/2, L/2-dx])
    plt.colorbar()
    plt.savefig('plots/000.svg')
    plt.close()
    plt.imshow(np.imag(r), cmap=plt.get_cmap("BuPu"), origin='lower', 
                extent=[-L/2, L/2-dx, -L/2, L/2-dx])
    plt.colorbar()
    plt.savefig('plots/001.svg')
    plt.close()
    print("hallo")
    psi = r*grey_soliton(0.,-10)*grey_soliton(-0.,10)
    psi = TimeEvolution(psi, 100001, dt_=dt)
